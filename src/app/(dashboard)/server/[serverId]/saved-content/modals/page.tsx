...
export default function SavedModalsPage() {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-[#f1f1f2]">Saved Modals</h1>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {modals.map((modal) => {
            ...
            return (
              <Card key={modal.id} className="group relative p-4 hover:border-[#5865F2]/50 transition-colors">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {fieldCount} field{fieldCount !== 1 ? "s" : ""} - Created {formatDate(modal.createdAt)}
                      </p>
                    </div>
                  </div>
                </Link>
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-[#3f4147] border-dashed py-16 text-center">
          <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-[#5865F2]/20">
            <LayoutTemplate className="h-6 w-6 text-[#5865F2]" />
          </div>
