...
function AutomationCard({
  ...
  return (
    <Card className="rounded-[4px] border border-[#3C3F45] bg-[#2B2D31] p-4 hover:border-[#5865F2]/40 transition-colors">
...
export default function SavedAutomationsPage() {
  ...
  const handleCreate = () => {
    router.push(`/server/${serverId}/automations`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && !sequences?.length && <EmptyState onCreate={handleCreate} />}
    </div>
  );
}